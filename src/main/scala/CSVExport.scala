...
object CSVExport {
  ...
  def dimensions(dimension: String, dimensions: Seq[Map[String,Int]]) : Option[Int] = {
    Option(dimensions) match {
      case Some(wrapped) => {
        wrapped.map(Option(_)).find(ds => !(ds.isEmpty)) match {
          case Some(Some(dimMap)) => dimMap.get(dimension)
          case _ => None
        }
      }
      case None => None
